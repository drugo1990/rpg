...
namespace Yukar.Battle
{
    ...
    public class BattleEventController : BattleEventControllerBase
    {
        ...
        public override void healBattleCharacter(Script.Command curCommand, Guid evGuid)
        {
            ...
            {
                var info = catalog.getGameSettings().GetCastStatusParamInfo(idAttr.GetGuid());
            ...
            {
                var info = catalog.getGameSettings().GetCastStatusParamInfo(GameSettings.MaxMPStatusID, true);
            ...
            {
                statusId = GameSettings.MaxHPStatusID;
            }
            ...
            if (target != null)
            {
                ...
                target.consumptionStatusValue.Min(statusId, target.GetSystemStatus(catalog.getGameSettings(), statusId));
                ...
                if (statusId == Common.Rom.GameSettings.MaxHPStatusID)
                {
				...
				else if (statusId == catalog.getGameSettings().GetNewSystemStatusId(Common.Rom.GameSettings.MaxMPStatusID))
                {
            ...
            if (target != null && showDamage)
            {
                ...
                battle.battleViewer.AddDamageTextInfo(new BattleDamageTextInfo(textType, target, text, statusId, catalog.getGameSettings().GetStatusColor(statusId, (textType == BattleDamageTextInfo.TextType.Damage))));
            }
        }
            ...
            foreach (var chr in tgts)
            {
                if (tgtIsMp)
                {
                    ...
                    if (chr.MagicPoint > chr.MaxMagicPoint)
                        ...
                        chr.MagicPoint = 0;
                    chr.consumptionStatusValue.SetStatus(GameSettings.MaxMPStatusID, chr.MagicPoint);
                }
                else
                {
                    chr.HitPoint += value;
                    if (chr.HitPoint > chr.MaxHitPoint)
                        ...
                        chr.HitPoint = 0;
                    chr.consumptionStatusValue.SetStatus(GameSettings.MaxHPStatusID, chr.HitPoint);
        ...
        public override void setBattleStatus(Script.Command curCommand, Guid evGuid, bool typeReduced = false)
        {
            ...
            foreach (var tgt in tgts)
            {
                if (add)
                {
                    ...
                    {
                        tgt.HitPoint = 0;
                        tgt.conditionStatusValue.SetStatus(GameSettings.MaxHPStatusID, 0);
                ...
                else
                {
                    if (condition.deadCondition && condition.deadConditionPercent == 0)
                    {
                        ...
                        {
                            tgt.HitPoint = 1;
                            tgt.conditionStatusValue.SetStatus(GameSettings.MaxHPStatusID, 1);
                            tgt.Resurrection();
                            battle.SetNextBattleStatus(tgt);
                        }
                    }
        ...
        public void consistency(BattlePlayerData p)
        {
            ...
            {
                p.HitPoint = 0;
                p.conditionStatusValue.SetStatus(GameSettings.MaxHPStatusID, 0);
                p.Down(catalog, this);
            }
